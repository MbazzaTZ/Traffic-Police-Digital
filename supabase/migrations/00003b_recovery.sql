-- =========================================================
-- TPDOP – 00003 Recovery / Idempotent Re-apply
-- =========================================================
-- PURPOSE
-- -------
-- If `00003_runtime_tables.sql` failed partway through with an
-- error like "column court_case_id does not exist", your DB is
-- in a partial state: some of the 12 new tables exist (possibly
-- with missing columns), others don't exist at all.
--
-- This script is FULLY IDEMPOTENT. It will:
--   1. CREATE TABLE IF NOT EXISTS for each of the 12 tables
--   2. ALTER TABLE ADD COLUMN IF NOT EXISTS for every column
--      (this is the key fix — adds missing columns to existing tables)
--   3. CREATE SEQUENCE IF NOT EXISTS
--   4. CREATE INDEX IF NOT EXISTS
--   5. Enable RLS, create policies, create triggers
--   6. Seed fine_schedule
--
-- Run this INSTEAD OF re-running 00003. After it succeeds, continue
-- with 00004_column_align.sql → 00005 → 00006.
--
-- It is safe to run this multiple times — every statement is
...
-- DONE
-- ══════════════════════════════════════════════════════════
select 'TPDOP 00003 RECOVERY COMPLETE' as status,
       '12 tables ensured with all columns + RLS + triggers + 20 fine_schedule seeds' as summary;
