...
export default function Sidebar({ isMobile = false, open = false, onClose }) {
  ...
  // On mobile: hide entirely until opened. Slide in from left + backdrop.
  // On desktop: fixed visible left rail.
  const sidebarStyle = {
  ...
  return (
    <>
      {isMobile && open && (
        <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.45)", zIndex:49, backdropFilter:"blur(2px)" }}/>
      )}
      <aside style={sidebarStyle}>
        <div style={{ padding:"20px 16px 16px", borderBottom:"1px solid rgba(255,255,255,.07)", textAlign:"center", position:"relative" }}>
          {isMobile && (
            <button onClick={onClose} style={{ position:"absolute", top:12, right:12, width:32, height:32, borderRadius:8, border:"none", background:"rgba(255,255,255,.08)", color:"white", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
              <X size={16}/>
            </button>
          )}
