...
import { Users, Building2, Shield, MapPin, UserPlus, Activity, Clock, CheckCircle } from "lucide-react";
import { useAppData } from "../../context/AppDataContext";

const ROLE_LABELS = {
  regular_officer:"Regular Officer", traffic_officer:"Traffic Officer",
  cid_officer:"CID Officer", forensic_officer:"Forensic Officer",
  ocs:"OCS", ocd:"OCD", rpc:"RPC", igp:"IGP", admin_officer:"Admin Officer",
};
...
export default function AdminDashboard() {
  ...
  const recentOfficers = [...officers].slice(0, 5);
  ...
  const kpis = [
    ...
    { label:"Regions Added",    sw:"Mikoa",              color:"#059669", icon:MapPin,    v: regions.length },
    { label:"Active Roles",     sw:"Majukumu",           color:"#D97706", icon:Shield,    v: 9 },
    { label:"Active Officers",  sw:"Maafisa Wanaofanya Kazi", color:"#16A34A", icon:CheckCircle, v: officers.filter(o=>o.status==="Active").length },
  ];
    ...
    <AdminLayout pageTitle="Dashboard" pageTitle2="Dashibodi ya Usimamizi">
      ...
      <div style={{ background:"linear-gradient(135deg,#03102B,#05193E,#082A63)", borderRadius:18, padding:"22px 28px", color:"white", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:16, marginBottom:22, boxShadow:"0 8px 28px rgba(3,16,43,.35)", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", gap:10, zIndex:1 }}>
          <button onClick={() => nav("/admin/create-user")} style={{ padding:"10px 20px", borderRadius:10, border:"none", background:"#FFD700", color:"#03102B", fontWeight:800, fontSize:13, cursor:"pointer" }}>
            + Create Officer
          </button>
          <button onClick={() => nav("/admin/stations")} style={{ padding:"10px 20px", borderRadius:10, border:"none", background:"rgba(255,255,255,.15)", color:"white", fontWeight:700, fontSize:13, cursor:"pointer" }}>
            + Add Station
          </button>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        ...
        <div style={{ background:"white", borderRadius:16, border:"1px solid #E2E8F0", overflow:"hidden" }}>
          <div style={{ padding:"14px 18px", borderBottom:"1px solid #F1F5F9", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              ...
              <div style={{ fontSize:11, color:"#94A3B8" }}>{officers.length} registered</div>
            </div>
            <button onClick={() => nav("/admin/officers")} style={{ background:"white", border:"1px solid #E2E8F0", borderRadius:8, padding:"6px 12px", fontSize:12, fontWeight:600, cursor:"pointer", color:"#475569" }}>
              View All
            </button>
          </div>
          ...
          {officers.length === 0 ? (
            <div style={{ padding:"40px 20px", textAlign:"center", color:"#94A3B8" }}>
              ...
              <button onClick={() => nav("/admin/create-user")} style={{ marginTop:12, padding:"8px 18px", borderRadius:10, border:"none", background:"#0D3477", color:"white", fontWeight:700, cursor:"pointer", fontSize:12 }}>
            ...
            <table style={{ width:"100%", borderCollapse:"collapse" }}>
              ...
              <tbody>
                {recentOfficers.map((o, i) => (
                  <tr key={o.id} style={{ borderBottom: i < recentOfficers.length-1 ? "1px solid #F1F5F9" : "none" }}
                    ...
                    <td style={{ padding:"11px 14px" }}>
                      <span style={{ background:"#F0FDF4", color:"#16A34A", padding:"3px 9px", borderRadius:999, fontSize:11, fontWeight:700 }}>Active</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ background:"white", borderRadius:16, border:"1px solid #E2E8F0", overflow:"hidden" }}>
            <div style={{ padding:"14px 18px", borderBottom:"1px solid #F1F5F9", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                ...
                <div style={{ fontSize:11, color:"#94A3B8" }}>{stations.length} registered</div>
              </div>
              <button onClick={() => nav("/admin/stations")} style={{ background:"white", border:"1px solid #E2E8F0", borderRadius:8, padding:"6px 12px", fontSize:12, fontWeight:600, cursor:"pointer", color:"#475569" }}>
                View All
              </button>
            </div>
            {stations.length === 0 ? (
              <div style={{ padding:"28px 20px", textAlign:"center", color:"#94A3B8" }}>
                ...
                <div style={{ fontSize:13, fontWeight:600, color:"#64748B" }}>No stations added yet</div>
                <button onClick={() => nav("/admin/stations")} style={{ marginTop:10, padding:"7px 16px", borderRadius:8, border:"none", background:"#0D3477", color:"white", fontWeight:700, cursor:"pointer", fontSize:12 }}>
                  Add Station
                </button>
              </div>
            ) : (
              <div style={{ padding:"10px 14px" }}>
                {recentStations.map((s, i) => (
                  <div key={s.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 6px", borderBottom: i<recentStations.length-1?"1px solid #F8FAFC":"none" }}>
                    <div>
                      <div style={{ fontSize:13, fontWeight:700, color:"#1E293B" }}>{s.name}</div>
                      <div style={{ fontSize:11, color:"#94A3B8" }}>{s.region} · {s.district||"—"}</div>
                    </div>
                    <span style={{ background:"#EFF6FF", color:"#0D3477", padding:"3px 9px", borderRadius:999, fontSize:10, fontWeight:700 }}>{s.type}</span>
                  </div>
                ))}
                {stations.length > 3 && (
                  <div style={{ padding:"8px 6px 2px", fontSize:12, color:"#0D3477", cursor:"pointer", fontWeight:600 }} onClick={() => nav("/admin/stations")}>
          ...
          <div style={{ background:"white", borderRadius:16, border:"1px solid #E2E8F0", padding:"16px 18px" }}>
            <div style={{ fontSize:14, fontWeight:700, color:"#03102B", marginBottom:12 }}>System Summary</div>
            {[
              { label:`${officers.length} Officer${officers.length!==1?"s":""} created`, c:"#0D3477", v:officers.length },
              { label:`${stations.length} Station${stations.length!==1?"s":""} registered`, c:"#059669", v:stations.length },
